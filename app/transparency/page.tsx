      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="rounded-xl p-6 mb-8 card-border" style={{ background: '#0F172A' }}>
          ...
          <div className="space-y-3">
            {[
              ...
              { field: 'confidence', desc: 'Deterministic 0.0–1.0 score computed from 30 days of price history (stability, market depth, sample size, volatility). A signal is only delivered once confidence reaches 0.70.' },
        ...
        <div className="rounded-xl p-6 mb-8 card-border" style={{ background: '#0F172A' }}>
          ...
          <p className="text-refinex-secondary text-sm mb-4">
            ...
            Bands are recalculated every cycle based on the deterministic confidence score — a signal that scores HIGH
            may be reclassified to WATCH if volatility increases before delivery.
          </p>
          <div className="space-y-3">
            {[
              { band: 'HIGH', range: '>= 0.75', color: '#10B981', border: 'rgba(16,185,129,0.3)', bg: 'rgba(16,185,129,0.08)', desc: 'Delivered to autoscaler. Price spread is significant, false positive risk is low. Act on this.' },
              { band: 'MEDIUM', range: '0.50 - 0.74', color: '#F59E0B', border: 'rgba(245,158,11,0.3)', bg: 'rgba(245,158,11,0.08)', desc: 'Delivered with advisory flag. Conditions are favourable but confidence is not yet in the HIGH band. Monitor closely.' },
        ...
        <div className="rounded-xl p-6 card-border" style={{ background: '#0F172A' }}>
          ...
          <p className="text-refinex-secondary text-sm mb-4">
            ...
            initial detection but fail one of three quality gates before delivery.
          </p>
          <div className="space-y-4">
            {[
              { title: 'Confidence below threshold', desc: 'Signal confidence score fell below the 0.70 delivery threshold. A real price spread was present, but the deterministic score did not clear the bar for delivery.' },
              ...
              { title: 'TTL expired', desc: "The signal's delivery window closed before it was acted on. Spot market conditions move fast enough that an unacted-on signal stops being valid." },
            ].map(item => (
